    ...
    for pasta in pastas.iter() {
        if pasta.id == id {
            if !pasta.editable {
                return Ok((
                    StatusCode::FOUND,
                    [(header::LOCATION, format!("{}/", data.args.public_path_as_str()))],
                    "".to_string(),
                )
                    .into_response());
            }
                    ...
                    if pastas[i].readonly || pastas[i].encrypt_server {
                            ...
                            && let Some(encrypted_key) = pastas[i].encrypted_key.as_ref()
                        {
                            ...
                            if res.is_ok() {
                                ...
                                // save pasta in database
                                update(Some(&pastas), Some(&pastas[i]), &data.args);
                            } else {
                                return Ok((
                                    StatusCode::FOUND,
                                    [(
                    ...
                    return Ok((
                        ...
                        [(
                            ...
                            format!(
                                "{}/upload/{}",
                                data.args.public_path_as_str(),
                                pastas[i].id_as_animals(&data.args)
                            ),
                        )],
                    )
                        .into_response());
