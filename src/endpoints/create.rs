    ...
    while let Some(mut field) = payload.next_field().await? {
        ...
        match field_name {
            ...
            "file" => {
                ...
                log::debug!(
                    "Starting file upload for: {}, max sizes - encrypted: {}MB, unencrypted: {}MB",
                    file.name(),
                ...
                while let Some(chunk) = field.try_next().await? {
                    ...
                    if current_mb > last_logged_mb {
                        log::debug!(
                            "File upload progress: {} bytes ({:.2}MB)",
                            size,
                    ...
                    {
                        log::warn!(
                            "File size limit exceeded: {} bytes ({}MB), encrypt_server: {}",
                            size,
                            size / (1024 * 1024),
                ...
                log::debug!(
                    "File upload completed: {} bytes ({:.2}MB) written to disk",
                    size,
                    size as f64 / (1024.0 * 1024.0)
                );
